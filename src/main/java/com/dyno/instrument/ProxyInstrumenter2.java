...
public class ProxyInstrumenter2 extends AstInstrumenter {
    ...
    public  boolean visit(AstNode node){
        ...
        /*	if (tt == org.mozilla.javascript.Token.FUNCTION) {
			String[] varsInScope = getVariablesNamesInFunction(((FunctionNode) node).getDefiningScope());
			System.out.println("++++++++++++++++++++++++++++");
			System.out.println("Variable declarations:");
			for (int i = 0; i < varsInScope.length; i++) {
				System.out.println(varsInScope[i]);
			}
			System.out.println("++++++++++++++++++++++++++++");
        ...
        if (tt == org.mozilla.javascript.Token.NAME
            ...
            // TODO: cannot find defining scope when global var used in level 1 function for some reason
            if (((Name) node).getDefiningScope() == null) {
                String[] varsInScope;

                ArrayList<Scope> sc = getScopeChain(variableName, node);
                Iterator<Scope> is = sc.iterator();
                Scope nextScope;
                boolean found = false;

                while (is.hasNext() && !found) {
                    nextScope = is.next();
                    varsInScope = getVariablesNamesInFunction(nextScope);
                    System.out.println("++++++++++++++++++++++++++++");
                    System.out.println("Variable declarations:");
                    for (int i = 0; i < varsInScope.length; i++) {
                        System.out.println(varsInScope[i]);
                ...
                if (found) {
                    if (nextScope.getType() ==  org.mozilla.javascript.Token.FUNCTION) {
                        ((FunctionNode) nextScope).getName(); // This name might be anonymous, use custom name getter ^^
                    ...
                    // in another .js file
                    System.out.println("[ProxyInstrumenter2-205]: Variable declaration not found for "
                ...
                System.out.println(Token.typeToName(((Name) node).getDefiningScope().getType()));	
            }


            System.out.println((((Name) node).getDefiningScope() instanceof FunctionNode));

            System.out.println(Token.typeToName(node.getAstRoot().getType()));
            ...
            /*		if (node.getParent() != null) {
				...
				System.out.println(node.getParent().getLineno());
				System.out.println(node.getParent().toSource());
				System.out.println("----------------------------------");
			}*/
        } else {
            // TODO: matching vaiable not found for parameters
        }
    ...
    private ArrayList<Scope> getScopeChain(String variableName, AstNode node) {
