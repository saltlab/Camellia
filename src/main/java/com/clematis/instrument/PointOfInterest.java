		...
		case org.mozilla.javascript.Token.FUNCTION: 
			if (this.getEnd() == -1) {
				...
				if (vars.length() > 0) {
					...
					code =  "send(JSON.stringify({messageType: \"FUNCTION_ENTER\", timeStamp: Date.now(), targetFunction: \""+getName()+"\",lineNo: "+getLineNo()+", scopeName: \""+getScopeName()+"\", counter:traceCounter++}));";
