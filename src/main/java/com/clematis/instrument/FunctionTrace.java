...
public class FunctionTrace extends AstInstrumenter {
	...
	private void handleFunction(FunctionNode node) {
		...
		AstNode beginningNode = parse(beginningPOI.toString(), lineNo);

		AstNode endingNode = parse(endingPOI.toString(), lineNo);
	...
	private void handleFunctionCall(FunctionCall node) {
		...
		if (targetBody.equals("this")) {
			// Square bracket notation cannot replace dot notation for 'this' keyword?
			return;
		}
	...
	private void handleReturn(ReturnStatement node) {
		...
		if (node.getReturnValue() != null) {
			// Wrap return value
//			newRV = parse("RSW("+ node.getReturnValue().toSource() + ", '" + node.getReturnValue().toSource().replace("'", "\\'")+ "' ," + lineNo +");", lineNo);
			newRV = parse("RSW("+ node.getReturnValue().toSource() + ", '" + "' ," + lineNo +");", lineNo);
