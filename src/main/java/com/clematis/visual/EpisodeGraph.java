...
public class EpisodeGraph {
	...
	public void createGraph() {
		...
		for (int i = 0; i< el.size(); i++) {
			...
			// Assertion causal links
			if (currentEpisode.getRelatedAssertions().size() > 0) {
				Vector<Integer> assertionsToLink = currentEpisode.getRelatedAssertions();
				// Iterate through related assertions
				for (int k = 0; k < assertionsToLink.size(); k++){
					// Find them in the story, add a link
					for (int j = 0; j< el.size(); j++) {
						if (el.get(j).getSource() instanceof SeleniumAssertionTrace
								&& ((SeleniumAssertionTrace) el.get(j).getSource()).getAssertionID() == assertionsToLink.get(k)) {
							addEdge(currentEpisode, 
									el.get(j), 
									el.get(j).getSource());
						}
