    ...
    private static void extraxtTraceObjects() {
        ...
        try {
            ...
            for (Episode e : story.getEpisodes()) {
                ...
                int closestCounterEnter = 123123;
                int closestCounterCall = 123123;
                ...
                while (assertions.hasNext()) {
                    ...
                    if (relatedMutations.size() > 0) {
                        ...
                        for (int k = 0; k < relatedMutations.size(); k++) {
                            ...
                            FunctionExit closestFunction = null;
                            // NOT SURE IF THE TRACE IS ORDERED!
                            for (int closest = 0; closest < e.getTrace().getTrace().size(); closest++) {
                                if ((e.getTrace().getTrace().get(closest).getCounter() < relatedMutations.get(k).getCounter())
                                        && e.getTrace().getTrace().get(closest) instanceof FunctionExit) {
                                    closestFunction = ((FunctionExit) e.getTrace().getTrace().get(closest));
                                } else if (e.getTrace().getTrace().get(closest).getCounter() > relatedMutations.get(k).getCounter()) {
                                    break;
                                }
                            }
                            if (closestFunction != null) {
                                System.out.println("-----------------------------------");
                                System.out.println("Closest Function based on ONLY mutation:");
                                System.out.println(closestFunction.getCounter());
                                System.out.println(closestFunction.getTargetFunction());
                                System.out.println(closestFunction.getLineNo());
                                System.out.println(closestFunction.getScopeName());
                                System.out.println("Related mutation:");
                                System.out.println(relatedMutations.get(k).getCounter());

                                DOMElementAccess closestAccess = null;
                                for (Episode e2 : story.getEpisodes()) {
                                    ArrayList<TraceObject> episodeDomAccesses2 = e2.getDomAccesses();

                                    for (TraceObject doa: episodeDomAccesses2) {
                                        if (((DOMElementAccess) doa).getXpath() != ""
                                                && ((DOMMutationTrace) (relatedMutations.get(k))).getParentNode().has("xPath")
                                                && ((DOMMutationTrace) (relatedMutations.get(k))).getParentNode().getString("xPath").indexOf(((DOMElementAccess) doa).getXpath()) == 0
                                                && ((DOMMutationTrace) (relatedMutations.get(k))).getCounter() > ((DOMElementAccess) doa).getCounter()) {
                                            closestAccess = (DOMElementAccess) doa;

                                        }
                                    }
                                }
                                System.out.println(".....................................");
                                System.out.println("Closest Relevant ACCESS based on xPath:");
                                if (closestAccess != null) {
                                    System.out.println(closestAccess.getCounter());
                                } else {
                        ...
                        while (elKeys.hasNext()){
                            ...
                            for (int tt = 0; tt < episodeDomAccesses.size(); tt++) {
                                ...
                                if (compareHTMLElements(singleAccess, new JSONObject(((DOMElementAccess) episodeDomAccesses.get(tt)).getElement()))) {
                                    ...
                                    closestCounterEnter = 123123;
                                    closestCounterCall = 123123;

                                    for (int f = 0; f < episodeTrace.size(); f++) {
                                        if (episodeDomAccesses.get(tt).getCounter() - episodeTrace.get(f).getCounter()  > 0
                                        ...
                                        } else if (episodeDomAccesses.get(tt).getCounter() - episodeTrace.get(f).getCounter()  > 0
                                                //&& episodeTrace.get(f).getCounter() - episodeDomAccesses.get(tt).getCounter() < closestCounter
                                    ...
                                    for (int ff = 0; ff < episodeTrace.size(); ff++) {
                                        if (episodeTrace.get(ff).getCounter() == closestCounterCall) {
                                            ...
                                            System.out.println("Assertion: " + assertionID);
                                            System.out.println(((FunctionCall) episodeTrace.get(ff)).getTargetFunction());
                                            System.out.println(((FunctionCall) episodeTrace.get(ff)).getCounter());
                                            break;
                                        }
                                    }
