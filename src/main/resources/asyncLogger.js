...
var getHTMLAsJSON = function(obj) {
    var parentNodeValue = JsonML.fromHTML(obj);
    if (parentNodeValue == null) {
        parentNodeValue = new Object();
        parentNodeValue.tagName = "unknown";
        parentNodeValue.child = "unknown";
...
Document.prototype.getElementById= function(id) {
    ...
    send(JSON.stringify({
        ...
        xpath: getPathTo(r),
        element: JSON.stringify(getHTMLAsJSON(r))
    }));
    return r;
}
...
Document.prototype.getElementsByTagName= function(id) {
	var date = Date.now();
    var r = geetTagName.call(this, id);
...
Document.prototype.getElementsByClassName= function(id) {
	var date = Date.now();
    var returnMe =  geetClassName.call(this, id);
    ...
    send(JSON.stringify({
        ...
        parameter: id,
        counter: traceCounter++,
        xpath: getPathTo(returnMe[0]),
        element: JSON.stringify(getHTMLAsJSON(returnMe))
    }));
    return returnMe;
}
...
var pageLoadFire = function (e) {
    window.removeEventListener("load", pageLoadFire, false);
	
    var date = Date.now();
    var jml = JsonML.fromHTML(document);
	if (jml) {
		jml = JSON.stringify(jml);
    }

 /*  	sendNewPageLoaded(JSON.stringify({
...
//window.addEventListener("load", pageLoadFire, false);
...
window.addEventListener("beforeunload", function (e) {
    window.console.log("Pushing buffer before unload!");
    sendReally();
}, false);
