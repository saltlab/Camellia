...
function handleSummary(summaries) {
	for (var s=0; s<summaries.length; s++){
		...
		for (var i=0; i<summaries[s].added.length; i++){
            ...
            if (parentNodeValue[0].toLowerCase() === 'script') {
			    added.push({
                    data: data,
                    nodeType: nodeType,
                    nodeName: nodeName,
                    nodeValue: nodeValue,
                    parentNodeValue: {
                        tagName: parentNodeValue[0].toLowerCase(),
                        child: parentNodeValue[1]
                    }
                });	
            } else {	
                parentNodeValue[1].tagName = parentNodeValue[0].toLowerCase();
                parentNodeValue[1].child = parentNodeValue[2];
				parentNodeValue[1].xPath = getPathTo(summaries[s].added[i].parentElement);
			    added.push({data: data, nodeType: nodeType, nodeName: nodeName, nodeValue: nodeValue, parentNodeValue: parentNodeValue[1]});	
            }
		}
		...
		for (var i=0; i<summaries[s].removed.length; i++){
            ...
            if (parentNodeValue[0].toLowerCase() === 'script') {
                ...
                parentNodeValue[1].tagName = parentNodeValue[0].toLowerCase();
                parentNodeValue[1].child = parentNodeValue[2];
//				parentNodeValue[1].xPath = getPathTo(summaries[s].getOldParentNode(summaries[s].removed[i]));
				/*if (summaries[s].getOldParentNode(summaries[s].removed[i]).childElementCount > 0) {
					parentNodeValue[1].childCount = summaries[s].getOldParentNode(summaries[s].removed[i]).childElementCount;	
				} */
			    removed.push({data: data, nodeType: nodeType, nodeName: nodeName, nodeValue: nodeValue, parentNodeValue: parentNodeValue[1]});	
            }
		}
