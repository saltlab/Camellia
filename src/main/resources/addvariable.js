...
var recordingInProgress = true;//true;//////////false; // Can use this for determining if Clematis should be logging or not
...
//        send(JSON.stringify({messageType: "RETURN_STATEMENT", timeStamp: date, returnValue: {label: arguments[1], value: arguments[0]}, lineNo: arguments[2], counter: traceCounter++}));
    	// todo
    	/*********************/
        send(JSON.stringify({messageType: "RETURN_STATEMENT", timeStamp: date, label: arguments[1], value: null /*arguments[0].toString()*/, lineNo: arguments[2], counter: traceCounter++}));
    } else {
    // arguments[0] = lineno
...
function sendNewPageLoaded(s){
	var wrapper = new Array();
    wrapper.push(s);
    window.xhr.open('POST', document.location.href + '?loadednewpage', false);
    window.xhr.send('['+wrapper.toString()+']');
}
