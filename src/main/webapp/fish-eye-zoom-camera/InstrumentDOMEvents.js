...
_detectedBrowser = (_detectedBrowser.indexOf("chrome") >= 0) ? "chrome" :
	(_detectedBrowser.indexOf("firefox") >= 0) ? "firefox" :
		(_detectedBrowser.indexOf("safari") >= 0) ? "safari" :
			(_detectedBrowser.indexOf("msie") >= 0) ? "internetexplorer" : undefined;
...
function overrideIndividualDOMLevel1(object) {
	...
	var j, currentEvent, originalLevel1Event, flag;
	
	// Get all possible level 1 events for this object
	var availableLevel1Events = getLevel1Events(object);

	// Modify Level 1 event registers ( 'onclick', 'onload', etc.)
	for (j = 0; j < availableLevel1Events.length; j++) {
		currentEvent = availableLevel1Events[j];
		flag = object.hasOwnProperty(currentEvent);
		originalLevel1Event = object[currentEvent];

		if (originalLevel1Event === undefined && currentEvent !== "onload") {
			// Set to null for now
			object[currentEvent] = null;
		}
		
		// Generate a replacement for original 'onclick', 'onload' mechanism
		if (!_clematisReplacementDescriptors.hasOwnProperty(currentEvent)) {
			_clematisReplacementDescriptors[currentEvent] = generateReplacementLevel1Event(currentEvent);
		}
		
		// Apply the above generate replace to the property for this object
		try {
			Object.defineProperty(object, currentEvent, _clematisReplacementDescriptors[currentEvent]);
		} catch (err) {
			window.console.log('[InstrumentDOMEvents]: Error replacing Level 1 DOM event.');
		}
		
		if (originalLevel1Event
				&& (Object.prototype.toString.call(originalLevel1Event) === '[object Function]')
				&& (originalLevel1Event.name === currentEvent)) {
			...
			var astRoot = esprima.parse(originalLevel1Event, {
				range: true,
				loc: true
			});
			...
			newArguments = originalLevel1Event.toString().substring(originalLevel1Event.toString().indexOf('(') + 1, originalLevel1Event.toString().indexOf(')'));
			newArguments = newArguments.split(',');

			// Call Expressions
			actualFunction = esmorph.modify(originalLevel1Event.toString(), esmorph.Tracer.CallExpression(callSnapshot));
			...
			newFn = new Function(
					"return function " + originalLevel1Event.name + "(" + newArguments.join(',') + "){" + newBody + "}"
			)();
			//originalLevel1Event = newFn;
		}
		if (originalLevel1Event || flag) {
			object[currentEvent] = originalLevel1Event;
		}

	}

	Object.defineProperty(object, "__em__ow", {
		value: 1,
		configurable: false,
		enumerable: false,
		writable: false
